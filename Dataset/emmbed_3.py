...
# ------------------- Configuration -------------------
# Increase MAX_WORKERS for more parallelism if your system/disk can handle it.
qdrant = QdrantClient(host="localhost", port=6333)
COLLECTION_NAME = "music_similarity"
BATCH_SIZE = 1000      # Number of points per upsert batch
MAX_WORKERS = 100      # Increase number of parallel threads
FILE_LIMIT = 500000   # Process up to 1.4 million files
...
def extract_features(json_path):
    ...
    # Use key_strength from "key_strength" or "key_temperley"
    ...
    # Retrieve metadata (artist, title)
...
def process_file(file_path):
    try:
        ...
        if len(norm_vector) != EXPECTED_DIM:
            print(f"Error: Normalized vector length {len(norm_vector)} does not match expected {EXPECTED_DIM} for file {file_path}. Skipping.")
            return None
        # For debugging, print the first file's raw and normalized vectors
        if process_file.debug_printed is False:
            print(f"\nFile: {file_path}")
            print("Raw vector:")
            print(vector)
            ...
            print(norm_vector)
            print("-----------------------------------------")
            process_file.debug_printed = True
        # Save both the normalized vector (for search) and raw vector in payload
        payload["normalized_vector"] = norm_vector
        return norm_vector, payload
    except Exception as e:
        print(f"Skipping {file_path} due to error: {e}")
        return None
process_file.debug_printed = False

# ------------------------------------------------------------------------------
# Index files in parallel and upsert in batches
# ------------------------------------------------------------------------------
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in futures:
            ...
            points.append({
                ...
                "payload": payload      # Payload includes both raw_vector and normalized_vector info
            })
            point_id += 1
            if point_id % 1000 == 0:
...
if __name__ == "__main__":
    JSON_FOLDER = "./"
    points = index_directory(JSON_FOLDER, file_limit=FILE_LIMIT)
